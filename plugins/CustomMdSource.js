...
module.exports = class CustomMdSource {
  ...
  constructor(api, options) {
    ...
    api.loadSource(async (actions) => {
      ...
      this.createSchemaTypes(actions);
      this.createResolvers(actions);
    });
  }
  ...
  createSchemaTypes({ addSchemaTypes, schema }) {
    const schemaTypes = this.options.schemaTypes;

    if (!schemaTypes) {
      return;
    }

    addSchemaTypes(schemaTypes.map(schema.createObjectType));
  }

  createResolvers({ addSchemaResolvers }) {
